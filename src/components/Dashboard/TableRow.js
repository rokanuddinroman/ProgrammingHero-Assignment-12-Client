...
const TableRow = ({ user, handleDelete, setShowModal, showModal }) => {
  ...
  const makeAdmin = () => {
    fetch(`https://qwerty-gi38.onrender.com/user/admin/${email}`, {
      method: "PUT",
      headers: {
  ...
  return (
    <div className="product-row flex items-center responsive-row">
      ...
      {role === "admin" ? (
        ...
        <button
          className="w-[90px] px-2 py-[2px] bg-gray-200 font-[500] rounded-[8px] text-gray-800"
          disabled
        >
          Member
        </button>
      ...
      <p className="flex justify-end">
        <button
          onClick={makeAdmin}
          className="text-blue-900 px-3 py-2 rounded-[30px] bg-blue-100 hover:bg-blue-200 flex  items-center disabled:bg-gray-200 text-gray-600"
          disabled={role === "admin" ? true : false}
        >
          <AiFillCrown
            color={role === "admin" ? "#8B929C" : "#3B82F6"}
            className="text-[18px]"
          />{" "}
          <p className="ml-1 font-[500] ">
            {role === "admin" ? "Promoted" : "Promote"}{" "}
          </p>
        </button>
