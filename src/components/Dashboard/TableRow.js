    ...
    return (
        <div style={{ gridTemplateColumns: "1fr 1fr 1fr 100px" }} className='product-row flex items-center'>
            ...
            <p className='quantity-column relative '>
                <span className=''>{user?.minimumOrderQuantity}</span>
            </p>
            ...
            <p className='flex justify-end'>
                <button onClick={makeAdmin} className='text-blue-900 px-3 rounded-[30px] bg-blue-100 hover:bg-blue-200 flex  items-center disabled:bg-gray-200 text-gray-600' disabled={role === "admin" ? true : false}><AiFillCrown color={role === "admin" ? "#8B929C" : "#3B82F6"} className='text-[18px]' /> <p className='ml-1 font-[500] '>{role === "admin" ? "Promoted" : "Promote"} </p></button>
                <button onClick={makeAdmin} className='icon-button bg-red-100 hover:bg-red-200'><AiFillDelete className='text-red-500 text-[18px]' /></button>
            </p>
        </div>
    );
};
