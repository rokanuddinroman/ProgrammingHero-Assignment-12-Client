...
const ManageOrders = () => {
  ...
  useEffect(() => {
    fetch("https://qwerty-gi38.onrender.com/orders")
      .then((res) => res.json())
      .then((data) => {
        setMyProducts(data);
        setSpinner(false);
  ...
  const handleShipping = (id) => {
    fetch(`https://qwerty-gi38.onrender.com/orders/${id}`, {
      method: "PUT",
      headers: {
              ...
              <p className="flex text-right justify-end">
                {product?.status !== "unpaid" && (
                  <button
                    ...
                    className="text-blue-900 px-3 rounded-[30px] bg-blue-100 py-2 px-4 hover:bg-blue-200 flex  items-center disabled:bg-gray-200 text-gray-600"
                    disabled={
                      product?.status === "shipped" ||
                      product?.status === "unpaid"
                    }
                  >
                    ...
                    <p className="ml-1 font-[500] ">
                      {product?.status === "shipped" ? "Shipped" : "Ship"}{" "}
                    </p>
                  </button>
                )}
