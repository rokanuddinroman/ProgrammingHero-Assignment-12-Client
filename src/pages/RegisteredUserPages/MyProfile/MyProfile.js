...
const MyProfile = () => {
    ...
    useEffect(() => {
        const url = `https://salty-shelf-96840.herokuapp.com/user/${user.email}`
        fetch(url, {
            headers: {
    ...
    const handleProfileUpdate = event => {
        ...
        fetch(url, {
            ...
            .then(data => {
                ...
                updateProfile({ displayName: event.target.displayName.value, photoURL: event.target.image.value });
                // event.target.reset()
            })
    }
    return (
            ...
            <div className='grid p-4 mt-4 border-[2px] border-gray-200 rounded-[8px] relative lg:flex'>
                ...
                <div>
                    <div className='flex justify-between items-center w-full'>
                        <span className='text-[30px] font-[600]'>{user.displayName || userData.displayName}</span>
                        {
            ...
            <form onSubmit={handleProfileUpdate} className='product-form mt-4'>
                <div className="product-form-container grid grid-cols-3 gap-4">
                    <div>
                        ...
                        <input onChange={e => setDisplayName(e.target.value)} className='w-[100%] rounded-[8px] bg-[#f6fbfe] p-[10px] mb-3 border-[2px] border-[#e3e8eb]' type="text" name="displayName" id="displayName" defaultValue={displayName || user.displayName || userData.displayName} required />
                    ...
                    <div>
                        ...
                        <input onChange={e => setImage(e.target.value)} className='w-[100%] rounded-[8px] bg-[#f6fbfe] p-[10px] mb-3 border-[2px] border-[#e3e8eb]' type="text" name="image" id="image" defaultValue={image || userData.image || user.photoURL} />
                    </div>
                </div>
